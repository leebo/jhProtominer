...
bool xptServer_sendBlockData(xptServer_t* xptServer, xptServerClient_t* xptServerClient)
{
	...
	{
		printf("xptServer_sendBlockData(): payloadNum out of range for worker %s", xptServerClient->workerName);
		return false;
	}
	// generate work
	...
	{
		printf("xptServer_sendBlockData(): Unable to generate work data for worker %s", xptServerClient->workerName);
		return false;
	}
	// build the packet
	bool sendError = false;
